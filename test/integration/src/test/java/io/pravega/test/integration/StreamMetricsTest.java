...
public class StreamMetricsTest {
    ...
    public void testStreamsAndScopesBasicMetricsTests() throws Exception {
        Collection<Counter> countersZero = MetricRegistryUtils.getCounters(MetricsNames.CREATE_SCOPE_FAILED);
        log.info("Counters Before@@@: {}", countersZero.size());
        for (val counter : countersZero) {
